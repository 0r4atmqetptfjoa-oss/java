...
fun AppNavigation(viewModel: MainViewModel) {
    ...
    NavHost(navController = navController, startDestination = MainMenuRoute) {
        ...
        composable<SettingsRoute> { SettingsScreen(navController = navController, soundEnabled = soundEnabled, musicEnabled = musicEnabled, hardModeEnabled = hardModeEnabled, onSoundChanged = { viewModel.toggleSound() }, onMusicChanged = { viewModel.toggleMusic() }, onHardModeChanged = { viewModel.toggleHardMode() }) }
        composable<GamesMenuRoute> { GamesMenuScreen(navController = navController) }

        navigation<AlphabetGraphRoute>(startDestination = AlphabetQuizRoute) {
            composable<AlphabetQuizRoute> { AlphabetGameScreen(onBackToMenu = { backToGames(navController) }) }
        }
